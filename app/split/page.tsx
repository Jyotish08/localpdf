...
export default function SplitPage() {
  ...
  const loadPdf = useCallback(
    async (uploaded: File) => {
      ...
      if (!valid) {
        setStatus("error");
        toast.error("This file is not a valid PDF.", { id: "split-load" });
        return;
      }
  ...
  const validateRange = (): string | null => {
    ...
    if (startPage > pageCount || endPage > pageCount) return `Pages must be between 1 and ${pageCount}.`;
  ...
  const handleSplitRange = async () => {
    ...
    if (validation) { toast.error(validation, { id: "split" }); return; }
    ...
    try {
      ...
      addFile({ filename, size: blob.size, timestamp: Date.now(), tool: "split" });
    } catch {
      setStatus("error");
  ...
  const handleSplitAll = async () => {
    ...
    try {
      ...
      addFile({ filename, size: zipBlob.size, timestamp: Date.now(), tool: "split" });
    } catch {
      setStatus("error");
      ...
      <main className="mx-auto w-full max-w-3xl flex-1 px-4 py-8 sm:px-6 sm:py-10">
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-2xl font-bold tracking-tight text-foreground sm:text-3xl">Split PDF</h1>
          </div>
          <p className="text-muted">Extract a page range or split every page — all on your device.</p>
        </div>
        ...
        {!hasFile && (
          <div
            ...
            className={`relative rounded-2xl border-2 border-dashed px-4 py-10 text-center transition sm:py-14 ${
              isDragActive ? "border-split bg-split/5" : "border-border bg-card hover:border-split/40 hover:bg-split/5"
            }`}
          >
            ...
            <p className="mt-4 text-xs text-muted">One PDF at a time. Files stay on your device.</p>
          </div>
        )}
        ...
        {hasFile && file && (
          <>
            ...
            {/* Tabs */}
            <div className="mt-6 flex rounded-xl border border-border bg-card p-1 shadow-sm" role="tablist" aria-label="Split mode">
              <button
                type="button" role="tab" aria-selected={mode === "range"}
                onClick={() => { setMode("range"); resetResult(); }}
                className={`flex-1 rounded-lg px-3 py-2.5 text-sm font-semibold transition sm:px-4 ${
                  mode === "range" ? "bg-background text-foreground shadow-sm ring-1 ring-border" : "text-muted hover:text-foreground"
                }`}
              >
                Page range
              </button>
              <button
                type="button" role="tab" aria-selected={mode === "all"}
                onClick={() => { setMode("all"); resetResult(); }}
                className={`flex-1 rounded-lg px-3 py-2.5 text-sm font-semibold transition sm:px-4 ${
                  mode === "all" ? "bg-background text-foreground shadow-sm ring-1 ring-border" : "text-muted hover:text-foreground"
                }`}
              >
                Split all pages
              </button>
            ...
            <div className="mt-6 rounded-2xl border border-border bg-card p-5 shadow-sm sm:p-6">
              {mode === "range" ? (
                <div role="tabpanel">
                  ...
                  <p className="mt-1 text-sm text-muted">Enter the first and last page to extract into a new PDF.</p>
                  <div className="mt-5 flex items-center justify-center gap-3">
                    <label className="sr-only" htmlFor="start-page">Start page</label>
                    <input
                      id="start-page" type="number" min={1} max={pageCount} value={startPage}
                      onChange={(e) => { setStartPage(Number(e.target.value)); resetResult(); }}
                    ...
                    <input
                      id="end-page" type="number" min={1} max={pageCount} value={endPage}
                      onChange={(e) => { setEndPage(Number(e.target.value)); resetResult(); }}
                      className="w-full max-w-[5.5rem] rounded-xl border border-border bg-background px-3 py-3 text-center text-lg font-semibold text-foreground outline-none transition focus:border-split focus:ring-2 focus:ring-split/20"
                    />
                  </div>
                  <p className="mt-3 text-center text-xs text-muted">Valid range: 1 – {pageCount}</p>
                </div>
              ) : (
                <div role="tabpanel">
        ...
        {isSplitting && (
          <div className="mt-8 rounded-2xl border border-border bg-card p-5 shadow-sm">
            ...
            <div className="mt-4 h-2 overflow-hidden rounded-full bg-border">
              <div className="h-full rounded-full bg-split transition-all duration-300" style={{ width: `${progress}%` }} />
            </div>
          </div>
        )}
        ...
        {isError && (
          <div className="mt-8 rounded-2xl border border-red-500/30 bg-red-500/5 p-5 shadow-sm">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-semibold text-foreground">Split failed</p>
                <p className="mt-1 text-sm text-muted">The PDF may be encrypted or corrupted. Please try another file.</p>
              </div>
            </div>
        ...
        {/* Actions */}
        {hasFile && (
          <div className="mt-8 flex w-full flex-col gap-3 pb-20 sm:flex-row sm:justify-end sm:pb-0">
            {isDone ? (
              <>
                <div className="flex w-full items-center gap-2 sm:w-auto">
                  <button
                    type="button"
                    onClick={() => { if (resultBlob) downloadBlob(resultBlob, resultFilename); }}
              ...
              <button
                type="button" onClick={handleSplit} disabled={!canSplit}
